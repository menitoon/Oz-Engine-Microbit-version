...
## Instancing the **Canvas**, **Camera**, and **Snake**

Let's create the base objects that we will use to make this:
```python
latency = 600.0
canvas = Canvas(0)
...
head = Sprite(canvas, 5, [2, 2], "head", group="snake")
snake_body.append(head)

body = Sprite(canvas, 3, [1, 2], "body", group="snake")
snake_body.append(body)
```

``snake_body`` is used to store all sprites that belongs to the snake and ``latency`` is time each frames lasts.
Now try rendering the screen with ``camera.send_light_update()``
you should see 2 dots in the middle of the screen.
...
def move(x, y):
  ...
  snake_body.append(head)
````
We will update this function later on.
...
## Updating moving function

Now that we can spawn apple we need to update it so we can detect collision with apples:
```python
def move(x, y):
  ...
  if len(colliding_objects) > 0:
    ...
    if "collectable" in colliding_objects:
      ...
      latency *= 0.95 #accelerates the game
      is_point = True
      score += 1
...
def check_death():
  ...
  position = canvas.get_sprite("head").position #gets the position of the "head"
...
Now that we have all of these functions we can make the ** gameloop**
...
def game():
  ...
  direction_input = direction #The direction from the frame before the current one
  camera.send_light_update()
  
  while not (button_a.is_pressed() == True or button_b.is_pressed() == True): #block until button "a" or "b" is pressed
      pass
  ...
  display.scroll("Score: " +
                 str(score)) if not score == 23 else display.scroll(
...
All we need to do now is use global in other functions:
...
And now it shoud be working if you still have a problem heres the full script:
````python
import math
